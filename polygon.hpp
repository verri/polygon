...
struct polygon_traits<
  Polygon, std::enable_if_t<std::conjunction_v<
...
template <typename Polygon, typename = void> struct has_free_vertices : std::false_type {};

template <typename Polygon>
struct has_free_vertices<Polygon,
                         std::void_t<decltype(vertices(std::declval<const Polygon&>()))>>
  : std::true_type
{
};
...
template <typename Polygon,
          typename Point = coord<typename polygon_traits<Polygon>::value_type>>
auto invoke_vertices(const Polygon& polygon) -> std::vector<Point>
{
  if
    constexpr(has_free_vertices_v<Polygon>) return vertices(polygon);
  else
...
private:
  ...
  public:
    ...
    template <typename Polygon> auto downcast() -> Polygon&
    {
      return dynamic_cast<Polygon&>(*this);
    }

    template <typename Polygon> auto downcast() const -> const Polygon&
    {
  ...
  public:
    ...
    {
      return std::is_base_of_v<convex_polygon_tag,
                               typename polygon_traits<Polygon>::polygon_category_tag>;
    }

    auto is_concave() const -> bool override
    {
      return std::is_base_of_v<concave_polygon_tag,
                               typename polygon_traits<Polygon>::polygon_category_tag>;
    }

    auto is_simple() const -> bool override
    {
      return std::is_base_of_v<simple_polygon_tag,
                               typename polygon_traits<Polygon>::polygon_category_tag>;
    }
    ...
    auto vertices() const -> std::vector<coord<value_type>> override
    {
      ...
      else
      {
        ...
        std::transform(
          vs.begin(), vs.end(), std::back_inserter(result),
    ...
    auto underlying_type() const -> std::type_index override { return typeid(Polygon); }
...
public:
  ...
  polygon(Polygon data)
    : data_{std::make_unique<erased<Polygon>>(std::move(data))}
  {
  }
  ...
  template <typename Polygon> decltype(auto) downcast()
  {
    return data_->template downcast<Polygon>();
  }

  template <typename Polygon> decltype(auto) downcast() const
  {
    return data_->template downcast<Polygon>();
  }
